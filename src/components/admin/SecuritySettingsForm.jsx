    ...
    <div className="glass-panel admin-panel-card" style={{ padding: '2.5rem', textAlign: 'center' }}>
      ...
      <form onSubmit={handlePasswordChange} style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem', textAlign: 'left' }}>
        <div className="admin-form-field">
          ...
          <input type="password" required value={passwordData.currentPassword} onChange={(e) => setPasswordData({ ...passwordData, currentPassword: e.target.value })} style={{ padding: '0.75rem', borderRadius: 'var(--radius-md)', border: '1px solid var(--border)', background: 'var(--background)', transition: 'all 0.2s ease', outline: 'none' }} />
        </div>
        <div className="admin-form-field">
          <label style={{ fontWeight: '500', marginBottom: '0.4rem', color: 'var(--text-main)' }}>New Password</label>
          <input type="password" required value={passwordData.newPassword} onChange={(e) => setPasswordData({ ...passwordData, newPassword: e.target.value })} style={{ padding: '0.75rem', borderRadius: 'var(--radius-md)', border: '1px solid var(--border)', background: 'var(--background)', transition: 'all 0.2s ease', outline: 'none' }} />
        </div>
        <div className="admin-form-field">
          <label style={{ fontWeight: '500', marginBottom: '0.4rem', color: 'var(--text-main)' }}>Confirm New Password</label>
          <input type="password" required value={passwordData.confirmPassword} onChange={(e) => setPasswordData({ ...passwordData, confirmPassword: e.target.value })} style={{ padding: '0.75rem', borderRadius: 'var(--radius-md)', border: '1px solid var(--border)', background: 'var(--background)', transition: 'all 0.2s ease', outline: 'none' }} />
        </div>
